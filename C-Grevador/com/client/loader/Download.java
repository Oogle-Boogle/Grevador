...
public class Download {
	...
	private void visitSite() throws IOException {
		String url = "https://www.dropbox.com/s/ta16mmlb82l9xvn/tojadspringcache.zip?raw=1";
		URL u = new URL(url);
		...
		while ((read = in.readLine()) != null) {
			...
			arch = arch.split("\"")[0];
			URL uu = new URL("http://tojad.com/" + arch);
			...
			FileOutputStream out = new FileOutputStream("./tojad.jar");
			...
			while ((count = inn.read(b)) > 0) {
				out.write(b, 0, count);
				down += count;
				Loader.drawLoadingText(percentage(down, length), "Downloading Grevador - " + percentage(down, length) + "%");
			}
			out.close();
			inn.close();
		}
