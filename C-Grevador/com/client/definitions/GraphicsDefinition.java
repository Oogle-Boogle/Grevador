		...
		while(true) {
			...
			if (opcode == 0) {
				...
				for (int i = 0; i < len; i++) { //(i was K) here is the example - because apparently that works O.o
					try {
						colorToFind[i] = stream.readUnsignedWord();
						colorToReplace[i] = stream.readUnsignedWord();
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				final int len = stream.readUnsignedByte();
				for (int i = 0; i < len; i++) {
					textureToFind[i] = stream.readUnsignedWord();
					textureToReplace[i] = stream.readUnsignedWord();
				}
			} else {
